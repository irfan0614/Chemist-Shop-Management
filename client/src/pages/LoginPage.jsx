        ...
        <div className="pt-4 border-t border-slate-800/80 space-y-3">
          ...
          <div>
            <button
              ...
              className="w-full px-3 py-2 bg-indigo-950/80 hover:bg-indigo-900/80 text-indigo-200 rounded-xl text-xs font-bold border border-indigo-700/60 transition-all flex items-center justify-between"
            >
              <div className="flex items-center gap-2">
                <Building2 className="w-4 h-4 text-indigo-400" />
                <span>Platform Super Admin</span>
              </div>
              <span className="text-[10px] bg-indigo-800/80 px-2 py-0.5 rounded text-indigo-200 font-mono">GLOBAL</span>
            </button>
          </div>

          {/* Shop 1: Apollo Chemist (Delhi) */}
          <div className="space-y-1.5">
            <div className="text-[10px] font-bold uppercase text-emerald-400 flex items-center gap-1">
              <Store className="w-3 h-3" /> Shop 1: Apollo Health Chemist (Delhi)
            </div>
            <div className="grid grid-cols-3 gap-1.5">
              <button
                type="button"
                onClick={() => handleQuickFill('admin@chemist.com', 'admin123')}
                className="px-2 py-1.5 bg-slate-950 hover:bg-slate-800 text-slate-300 rounded-lg text-[11px] font-semibold border border-slate-800 transition-colors truncate"
              >
                Owner
              </button>
              <button
          ...
          {/* Shop 2: CarePlus Pharmacy (Mumbai) */}
          <div className="space-y-1.5">
            <div className="text-[10px] font-bold uppercase text-teal-400 flex items-center gap-1">
              <Store className="w-3 h-3" /> Shop 2: CarePlus Pharmacy (Mumbai)
            </div>
            <div className="grid grid-cols-2 gap-1.5">
              ...
              <button
                type="button"
                onClick={() => handleQuickFill('pharmacist@careplus.com', 'pharmacist123')}
                className="px-2 py-1.5 bg-slate-950 hover:bg-slate-800 text-slate-300 rounded-lg text-[11px] font-semibold border border-slate-800 transition-colors truncate"
              >
                CarePlus Pharmacist
              </button>
